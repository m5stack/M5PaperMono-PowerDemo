...
namespace {
constexpr char kTag[]              = "imu_wake_deep";
...
[[noreturn]] void fail(const char* stage, const char* step, esp_err_t error, int raw_error)
{
    ...
    m5pm::test_support::report_final(m5pm::test_support::FinalStatus::fail, "imu_wake_deep_sleep_failed");
    m5pm::test_support::hold_safe_idle();
}

[[noreturn]] void finish_wake(const m5pm::test_support::TestContext& context, const m5pm::power::WakeStatus& pm1,
                              const m5pm::wakeup::ImuStatus& imu)
{
    ...
    const bool valid = context.wake_cause == ESP_SLEEP_WAKEUP_EXT0 && !unrelated_wake && gpio4_triggered &&
                       !unrelated_gpio && pm1.pm1_system_irq == 0U && pm1.pm1_button_irq == 0U && imu.any_motion;

    ESP_LOGI(kTag,
             "WAKE_CAPTURE source=imu esp=%s pm1=0x%02X gpio_irq=0x%02X "
             "sys_irq=0x%02X btn_irq=0x%02X imu=0x%04X any_motion=%d",
             m5pm::test_support::wake_cause_name(context.wake_cause), pm1.pm1_wake, pm1.pm1_gpio_irq,
             pm1.pm1_system_irq, pm1.pm1_button_irq, imu.raw, imu.any_motion);
    if (valid) {
        m5pm::test_support::report_final(m5pm::test_support::FinalStatus::pass, "imu_wake_deep_sleep");
    } else {
        ESP_LOGE(kTag,
                 "WAKE_FAIL expected_esp=ext0 expected_pm1_gpio4=1 "
                 "expected_imu_any_motion=1 unrelated_events_must_be=0");
...
extern "C" void app_main(void)
{
    ...
    if (context.wake_cause != ESP_SLEEP_WAKEUP_UNDEFINED) {
        finish_wake(context, retained_pm1, retained_imu);
    }

    power_result = power.prepare_imu_wake_l2();
    if (!power_result.ok()) {
        fail("l2_prepare", power_result.step, power_result.error, power_result.raw_error);
    }
    ...
    ESP_LOGI(kTag, "BOARD_OK PM1=1 IOE1=1 IMU=1 PROFILE=%s WAKE=imu", kImuWakeProfileName);
    ESP_LOGI(kTag, "WAKE_ARMED source=imu path=pm1_gpio4+aggregate_gpio1+esp_ext0");
    ...
    power_result = power.enter_imu_wake_l2();
