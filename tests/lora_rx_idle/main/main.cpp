...
extern "C" void app_main(void)
{
    ...
    if (radio.receive() != ESP_OK) m5pm::phase_test::fail("lora_receive", ESP_FAIL);
    ESP_LOGI("lora_rx_idle",
             "BOARD_OK BASELINE=l2-awake RADIO=sx1262 FREQ_MHZ=868 BW_KHZ=125 SF=12 CR=4/5 SYNC=0x34 TX_DBM=22 "
             "PREAMBLE=8 TCXO_V=3.0 REGULATOR=ldo STATE=continuous_rx TEST_PACKETS=0 TX=0 ACK=0 RETRY=0");
