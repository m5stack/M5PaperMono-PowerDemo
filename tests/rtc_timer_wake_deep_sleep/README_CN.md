...
本固件用于测量 RX8130CE RTC Timer 唤醒 ESP32-S3 Deep Sleep 状态时的整板输入电流。
...
| 唤醒验证 | Timer 标志置位、PM1 GPIO0 IRQ 存在、ESP32 唤醒原因为 EXT0 |
| 固件日志 | `BOARD_OK PM1=1 IOE1=1 RTC=1 WAKE=rtc_timer` 和 `WAKE_ARMED source=rtc_timer interval=120s path=pm1_gpio0+aggregate_gpio1+esp_ext0` |

## 运行模式

通用 RTC 测试读取保持的 RTC 和 PM1 标志，准备 L2 RTC Timer 唤醒链路，配置 120 s Timer，释放总线，输出 `BOARD_OK`、`WAKE_ARMED` 和 `TEST_READY`，等待 3000 ms 后进入 Deep Sleep。唤醒后验证 Timer 标志和 PM1 GPIO0 链路。
